        ...
        >
          ...
          >
            ...
            <q-input
              ...
              class="text-bold q-my-lg"
              inverted-light
              stack-label="Подтвердите пароль"
              type="password"
            ...
            >
              <q-checkbox
                :error="$v.form.agreementChecked.$error"
                @blur="$v.form.agreementChecked.$touch"
                :color="$v.form.agreementChecked.$error ? 'negative' : 'secondary'"
                :class="{ 'text-negative': $v.form.agreementChecked.$error }"
                checked-icon="ion-checkbox-outline"
                unchecked-icon="ion-square-outline"
                v-model="form.agreementChecked"
              >
                &nbsp; Я согласен с &nbsp;
              </q-checkbox>
    ...
    register () {
      ...
      this.$store.dispatch('auth/register', this.form)
        .then(response => {
          this.$q.notify({
            icon: 'ion-thumbs-up',
            message: response.data.message || 'Успешная регистрация и вход в личный кабинет.',
            timeout: 5000,
            type: 'positive'
          })
